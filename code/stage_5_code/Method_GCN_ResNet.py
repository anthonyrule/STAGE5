...
class MethodDeepGCNResNet(nn.Module):
    ...
    def train_model(self, epoch_iter):
        ...
        for epoch in range(epoch_iter):
            ...
            loss_val = F.nll_loss(output[self.train_test_val['idx_val']],
                                  self.data['y'][self.train_test_val['idx_val']])
            accuracy.data = {'true_y': self.data['y'][self.train_test_val['idx_val']],
                             'pred_y': output[self.train_test_val['idx_val']].max(1)[1]}
            acc_val = accuracy.evaluate()

            loss_test = F.nll_loss(output[self.train_test_val['idx_test']],
                                   self.data['y'][self.train_test_val['idx_test']])
    ...
    def test_model(self):
        ...
        return {'stat': {'test': {'loss': loss_test.item(), 'acc': acc_test}, 'train': {'loss': loss_train.item(), 'acc': acc_train}},
                'true_y': self.data['y'][self.train_test_val['idx_test']], 'pred_y': output[self.train_test_val['idx_test']].max(1)[1]}, acc_test.item()
